...
export default function Bilateral({ chapter }: { chapter: Chapter }) {
  return (
    <div className="content absolute inset-0" style={{display: 'grid', gridTemplateColumns: '52px 1fr' }}>
      ...
      {/* main */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', padding: '80px 64px 60px 44px', gap: '50px', alignItems: 'end' }}>
        <div>
          ...
          {{ fontFamily: 'var(--font-satoshi)', fontSize: '17px', lineHeight: 1.72, fontWeight: 700, color: 'rgba(248,246,242,0.45)', textAlign: 'justify' }}>
            {chapter.body}
          </p>}
        </div>
        {chapter.demo && <div className="flex flex-col justify-end"><DemoZone chapter={chapter} style={{ height: '300px', width: '100%' }} /></div>}
      </div>
    </div>
  );
}
